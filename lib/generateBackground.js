...
if (typeof window === 'object') {
  ...
  tessellation.cells.map((cell, i) => {
    ...
    const colorIndex = Math.floor(random(0, colors.length))    // const gradient = svg.gradient('linear', function(add) {
    //   add.stop(0, colors[colorIndex])
    //   add.stop(1, gradientColors[colorIndex])
    // })
